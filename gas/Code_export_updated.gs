 ...
 *     trainees: [{ videoId, videoTitle, trainee_name, trainee_name_ja, stage_type, playlist_id,
 ...
 * Code_collect.gs の MASTER_COLS と同一の定義を保持する。
 * ★ trainee_name_ja 列（D列）追加により、以降の列インデックスが +1 シフト。
 */
const MASTER_COLS_EX = {
  videoId:          0,
  videoTitle:       1,
  trainee_name:     2,
...
function _buildDashboardData() {
  ...
  const trainees = masterRows.map(row => {
    const views    = Number(row[MASTER_COLS_EX.viewCount])    || 0;
    const likes    = Number(row[MASTER_COLS_EX.likeCount])    || 0;
    const comments = Number(row[MASTER_COLS_EX.commentCount]) || 0;

    return {
      videoId:         String(row[MASTER_COLS_EX.videoId]         || ''),
      videoTitle:      String(row[MASTER_COLS_EX.videoTitle]       || ''),
      trainee_name:    String(row[MASTER_COLS_EX.trainee_name]     || ''),
      trainee_name_ja: String(row[MASTER_COLS_EX.trainee_name_ja]  || ''),  // ★ 新規追加
      stage_type:      String(row[MASTER_COLS_EX.stage_type]       || ''),
      publishedAt:     row[MASTER_COLS_EX.publishedAt]  ? String(row[MASTER_COLS_EX.publishedAt]).substring(0, 10)  : '',
      viewCount:       views,
      likeCount:       likes,
      commentCount:    comments,
      updatedAt:       row[MASTER_COLS_EX.updatedAt]    ? String(row[MASTER_COLS_EX.updatedAt]).substring(0, 10)    : '',
      playlist_id:     String(row[MASTER_COLS_EX.playlist_id]      || ''),
      // 計算指標
